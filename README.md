...
| **Lazaretto Scan** | [https://github.com/jamesdfinance-dev/lazaretto-scan-action](https://github.com/jamesdfinance-dev/lazaretto-scan-action) | GitHub Action (MIT) that fails the build when a pinned dependency is known malware. Checks package-lock, yarn.lock, and pnpm-lock against published advisories with no API key, and posts the verdict as a PR comment | ![Lazaretto](https://img.shields.io/github/stars/jamesdfinance-dev/lazaretto-scan-action?style=for-the-badge) |
